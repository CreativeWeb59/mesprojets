...
@section('content')
    <h1 class="h1">Statistiques CA</h1>
    ...
    <div class="w-auto h-auto flex justify-center border shadow mt-8 bg-white">
        <form action="{{ route('admStats.show') }}" method="POST" class="w-96">
            @csrf
            @method('POST')
            @include('partials.SelectPeriode')
            <input type="hidden" name="typeStats" value="statsCa">
        </form>
    </div>
